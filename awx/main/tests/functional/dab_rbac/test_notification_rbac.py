...
def test_creator_access_list_with_add_only_role(rando, organization, post, get, nt_add_role):
    """User with add_only role creates a notification template and has role assignments"""
    ...
    assignments = RoleUserAssignment.objects.filter(user=rando, content_type=ct, object_id=nt.pk)
...
def test_unpermissioned_user_cannot_access_access_list(rando, organization, post, admin_user, get, setup_managed_roles):
    """User without view permission has no role assignments on admin-created objects"""
    from django.contrib.contenttypes.models import ContentType
...
def test_access_list_shows_creator(rando, organization, post, get, nt_add_role, setup_managed_roles):
    """Creator has direct role assignments on their notification template"""
    from django.contrib.contenttypes.models import ContentType
